...
class ReCaptcha {
    ...
    init () {
        ...
        window.recaptchaSuccessCallback = (token) => this._eventBus.emit(
            "recaptcha-success",
            token
        );
        window.recaptchaExpiredCallback = () => this._eventBus.emit("recaptcha-expired");
        window.recaptchaErrorCallback = () => this._eventBus.emit(
            "recaptcha-error",
            "Failed to execute"
        );
...
export default function (_, inject) {
    ...
    Vue.component(
        "recaptcha",
        () => import("./recaptcha.vue")
    );
    inject(
        "recaptcha",
        new ReCaptcha(<%= serialize(options) %>)
    );
