...
export default async (context) => {
    ...
    const loadAndSetLocale = async (newLocale, {initialSetup = false} = {}) => {
        ...
        await syncVuex(
            newLocale,
            app.i18n.getLocaleMessage(newLocale)
        );
    ...
    app.i18n.__detectBrowserLanguage = async (route) => {
        ...
        if (detectBrowserLanguage) {
            ...
            if (useCookie && (browserLocale = getLocaleCookie()) && browserLocale !== 1 && browserLocale !== "1") {
                ...
                browserLocale = navigator.language.toLocaleLowerCase().substring(
                    0,
                    2
                );
                ...
                browserLocale = req.headers["accept-language"].split(",")[0].toLocaleLowerCase().substring(
                    0,
                    2
                );
