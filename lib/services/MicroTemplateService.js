 ...
 * @param {object} a The locations.
 * @param {object} b The locations.
 */
function intersectsLocs(a, b) {
    const uselessAEnd = a.end == null || eq(a.start, a.end)
    const uselessBEnd = b.end == null || eq(b.start, b.end)
    if (uselessAEnd && uselessBEnd) {
        return eq(a.start, b.start)
    }

    if (uselessAEnd) {
        return (
            (lt(b.start, a.start) || eq(b.start, a.start)) && lt(a.start, b.end)
        )
    }
    if (uselessBEnd) {
        return (
            (lt(a.start, b.start) || eq(a.start, b.start)) && lt(b.start, a.end)
        )
    }

    return lt(a.start, b.end) && lt(b.start, a.end)
}
...
class MicroTemplateService {
    ...
    intersectsTemplateTag(locs) {
        const rangeLocs = normalizeLocs(locs)
        for (const token of this.getMicroTemplateTokens()) {
