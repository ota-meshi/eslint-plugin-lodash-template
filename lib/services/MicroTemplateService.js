...
const htmlCommentDirective = require("./html-comment-directive")
const BranchedTemplateStore = require("./BranchedTemplateStore")
...
/**
 * Check if A greater than B (A > B)
 * @param {object} locA The location.
 * @param {object} locB The location.
 */
function gtLocs(locA, locB) {
 ...
 * @param {object} locsA The locations.
 * @param {object} locsB The locations.
 */
function intersectsLocs(locsA, locsB) {
    const maxStart = gtLocs(locsA.start, locsB.start)
        ? locsA.start
        : locsB.start
    const minEnd = gtLocs(locsA.end, locsB.end) ? locsB.end : locsA.end

    return gtLocs(minEnd, maxStart)
}
...
/**
 * Check whether the location is intersects token.
 * @param {object|[number, number]} locs The locations or indexes.
 * @param {object} token The token.
 * @returns {boolean} `true` if the location is in token.
 */
function intersectsToken(locs, token) {
    if (
        Array.isArray(locs) &&
        locs.length === 2 &&
        typeof locs[0] === "number" &&
        typeof locs[1] === "number"
    ) {
        const a = locs
        const b = token.range
        return Math.max(a[0], b[0]) < Math.min(a[1], b[1])
    }
    return intersectsLocs(locs, token.loc)
}
...
class MicroTemplateService {
     ...
     * @param {object|[number, number]} loc The locations or indexes.
     * @returns {boolean} `true` if the location is intersects template tag.
     */
    intersectsTemplateTag(locs) {
        const rangeLocs = this.normalizeLocs(locs)
        for (const token of this.getMicroTemplateTokens()) {
            if (intersectsToken(rangeLocs, token)) {
                return true
            }
        }
        return false
    ...
    isDisableMessageForHtml(message) {
        if (!this._htmlCommentDirectiveContext) {
            this._htmlCommentDirectiveContext = htmlCommentDirective.createCommentDirectiveContext(
                !this._doc,
                this.template,
                this
            )
    ...
    getBranchProcessedHtmlDocument(targetNode, sourceCode) {
        if (!this._branchedTemplateStore) {
            this._branchedTemplateStore = new BranchedTemplateStore(
                sourceCode.ast,
                sourceCode.visitorKeys,
                this.template
            )
        }
        if (!this._branchedTemplateStore.hasBranchStatements()) {
            return this.getDocument()
        }
        const branchedTemplate = this._branchedTemplateStore.getCoversBranchTemplate(
            targetNode.range[0]
        )
        if (!branchedTemplate.doc) {
            branchedTemplate.doc = this.parseHtml(branchedTemplate.template)
        }
        return branchedTemplate.doc
    }
    ...
    normalizeLocs(locs) {
        if (locs.line != null && locs.column) {
            // maybe message
            const messageLoc = { start: locs }
            if (
                locs.endLine != null &&
                locs.endColumn != null &&
                (locs.endLine !== locs.line || locs.endColumn !== locs.column)
            ) {
                messageLoc.end = {
                    line: locs.endLine,
                    column: locs.endColumn,
                }
            } else {
                const start = this.getIndexFromLoc(locs)
                messageLoc.end = this.getLocFromIndex(start + 1)
            }
            return messageLoc
        }
        return locs
