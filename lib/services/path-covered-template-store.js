...
class BranchContext {
    ...
    stripIfBlockOnUnusedBranches(pathCoveredTemplate, targetIndex) {
        ...
        const truthyNotConditions = new Map()
        const falsyConditions = new Map()

        /**
         * @param {IfStatement} node
         */
        const useConsequent = (node) => {
            for (const condition of this.getConditionsForTruthy(node.test)) {
                truthyNotConditions.set(condition.not.text, condition.not)
            }
            if (hasBranch(node)) {
         ...
         * @param {IfStatement} node
         */
        const useAlternate = (node) => {
            for (const condition of this.getConditionsForFalsy(node.test)) {
                falsyConditions.set(condition.text, condition)
            }
            pathCoveredTemplate.strip(
    ...
    *getConditionsWithoutCache(
    ...
    getConditionFromNode(node) {
        if (node.type === "UnaryExpression" && node.operator === "!") {
            const not = this.getConditionFromNode(node.argument)
            return not.not
        }
