...
module.exports = class BranchedTemplateStore {
    ...
    /**
     * Get the template that covers the branch that reaches the given targetIndex.
     * @param {number} targetIndex The target index.
     * @returns {BranchedTemplate} The template with branch covering targetIndex
     */
    getCoversBranchTemplate(targetIndex) {
    ...
    /**
     * Get all templates that cover all branches.
     * @returns {BranchedTemplate[]} all templates that cover all branches.
     */
    getAllTemplates() {
        const map = new Map()
        let index = 0
        while (this._template.length > index) {
            const template = this.getCoversBranchTemplate(index)
