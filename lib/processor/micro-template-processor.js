...
function postprocessForDisableRules(messages, microTemplateService) {
    ...
    return messages.filter(message => {
        if (
            microTemplateService.locationInTemplate(message) &&
            TEMPLATE_DISABLED_RULES.indexOf(message.ruleId) >= 0
        ) {
            return false
        }
        if (
            microTemplateService.locationInInterpolateOrEscape(message) &&
            TMPL_INTERPOLATE_DISABLED_RULES.indexOf(message.ruleId) >= 0
        ) {
            return false
        }
        if (
            microTemplateService.locationInDelimiterMarks(message) &&
