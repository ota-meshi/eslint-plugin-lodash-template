...
module.exports = {
    preprocess(code, filename) {
        if (sharedContainer.getBranchedTemplate(filename)) {
            // already processed
            return [code]
        }
        ...
        const codes = templates.map((branchedTemplate, index) => {
            const prefix = hash(branchedTemplate.template)
            const scriptFilename = `eslint-plugin-lodash-template-${prefix}${index}.js`

            container.addBranchedTemplate(scriptFilename, branchedTemplate)
            return {
                text: code,
