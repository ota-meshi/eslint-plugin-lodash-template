...
const TEMPLATE_DELIMITERS_DISABLED_RULES = [
    "semi-spacing",
    "semi",
    "no-extra-semi",
    "semi-style",
...
const TEMPLATE_INTERPOLATE_DISABLED_RULES = ["no-unused-expressions"];
...
function postprocessForDisableRules(filename, messages, microTemplateService) {
    ...
    return messages.filter((message) => {
        ...
        if (
            ...
            TEMPLATE_INTERPOLATE_DISABLED_RULES.indexOf(message.ruleId) >= 0
        ) {
            return false;
        }
        if (
            microTemplateService.inDelimiterMarks(message) &&
            TEMPLATE_DELIMITERS_DISABLED_RULES.indexOf(message.ruleId) >= 0
        ) {
            return false;
        }
