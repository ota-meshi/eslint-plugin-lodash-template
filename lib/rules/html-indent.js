...
module.exports = {
    meta: {
        docs: {
            ...
            url:
                "https://github.com/ota-meshi/eslint-plugin-lodash-template/blob/v0.8.0/docs/rules/html-indent.md",
        },
        fixable: "whitespace",
        schema: [
            {
    ...
    create(context) {
        ...
        function processAdjustEndTags() {
            ...
            for (const invalidTarget of invalidCandidateEndTags) {
                const doc = microTemplateService.getBranchProcessedHtmlDocument(
                    invalidTarget.endTag,
                    sourceCode
                )

                const node = microTemplateService.findToken(
                    doc,
                    n =>
                        n.type === "HTMLEndTag" &&
                        equalsLocation(invalidTarget.endTag, n)
                )
                if (!node) {
                    continue
                }
