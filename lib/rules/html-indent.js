...
module.exports = {
    meta: {
        docs: {
            ...
            url:
                "https://github.com/ota-meshi/eslint-plugin-lodash-template/blob/v0.0.6/docs/rules/html-indent.md",
        },
        fixable: "whitespace",
        schema: [
            {
    ...
    create(context) {
        ...
        function setOffsetForIndentEvaluates(startEvaluate, endEvaluate) {
            ...
            for (let lineNo = baseLineNo + 1; lineNo <= endLineNo; lineNo++) {
        ...
        function processEvaluates(evaluates) {
            ...
            adjustLastEvaluates.forEach((start, end) => {
                setOffset(end, 0, start.loc.start.line)
            })
        }
            ...
            "Program:exit"() {
                microTemplateService.traverseDocumentNodes({
                    HTMLElement(node) {
                        ...
                        setOffsetRootLineIndex(startToken.range[0])
                        if (node.name === "pre" || node.name === "script") {
                            ignoreRanges.push([
                                startToken.range[1],
                                (node.endTag || node).range[0],
                            ])
                            return
                        }
                        for (const n of node.children) {
                            if (n.type === "HTMLText") {
                                ...
                                setOffset(n, 1, startToken.loc.start.line)
                            }
                        }
                        if (node.endTag) {
                            setOffset(node.endTag, 0, startToken.loc.start.line)
                        }
                    },
                    HTMLStartTag(node) {
                        setOffset(
