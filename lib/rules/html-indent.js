...
module.exports = {
    meta: {
        docs: {
            ...
            url:
                "https://github.com/ota-meshi/eslint-plugin-lodash-template/blob/v0.0.10/docs/rules/html-indent.md",
        },
        fixable: "whitespace",
        schema: [
            {
    ...
    create(context) {
        ...
        function processAdjustEndTags() {
            ...
            const lineEndTags = new Map()
            microTemplateService.traverseDocumentNodes({
                HTMLEndTag(node) {
                    if (getBeforeTextInLine(node.loc.start).trim()) {
                        return
                    }
                    lineEndTags.set(node.loc.start.line, node)
                },
            })
            const invalidCandidateEndTags = []
            lineEndTags.forEach((tag, line) => {
                if (line === 0) {
                    return
                }
                const preLine = line - 1
                const preLineEndTag = lineEndTags.get(preLine)
                if (!preLineEndTag) {
                    return
                }
                const preLineIndent = getExpectedIndent(preLine, true)
                const curLineIndent = getExpectedIndent(line, true)
                if (preLineIndent !== curLineIndent) {
                    return
                }
                invalidCandidateEndTags.push(tag)
            })
            ...
            microTemplateService.traverseTokens(doc, {
                HTMLEndTag(node) {
                    const target = invalidCandidateEndTags.find(n =>
                        equalsLocation(n, node)
                    )
                    if (!target) {
                        return
                    }
                    ...
                    if (
                        target.parent.startTag &&
                        equalsLocation(startTag, target.parent.startTag)
                    ) {
                        // no change start tag location
                        return
                    }

                    setOffsetToToken(target, 0, startTag.loc.start.line)
                },
            })
        }
