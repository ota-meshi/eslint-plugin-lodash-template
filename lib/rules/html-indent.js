...
module.exports = {
    ...
    create(context) {
        ...
        function createOffsetDataFromLine(line, baseObject) {
            const text = sourceCode.getLine(line)
            const index = text.search(/\S/)
            return Object.assign(
                {
                    actualText: text.slice(0, index),
                    expectedIndent: undefined,
                },
                baseObject
            )
        ...
        /**
         * Find indentation-end punctuator token, within micro-template evaluate.
         * @param {Node} evaluate The micro-template evaluate to set
         * @param {string} openPunctuatorText The open punctuator.
         * @returns {Token} The indentation-end punctuator token.
         */
        function findIndentationEndPunctuator(evaluate, openPunctuatorText) {
            const searchIndex = evaluate.code.search(/\S/)
            if (searchIndex < 0) {
                return null
            }
            const charIndex = evaluate.expressionStart.range[1] + searchIndex
            ...
            if (!targetToken) {
                targetToken = tokens.find(t => charIndex < t.range[0])
            }
            const isIndentationEndPunctuator =
                openPunctuatorText === "("
                    ? isRightParen
                    : openPunctuatorText === "{"
                        ? isRightBrace
                        : isRightBracket
            if (!isIndentationEndPunctuator(targetToken)) {
                return null
            }
            return targetToken
        }
        ...
        function processEvaluates(evaluates) {
            evaluates.slice(0, -1).forEach((evaluate, index) => {
                const leftToken = findIndentationStartPunctuator(evaluate)
                if (!leftToken) {
                    return
                }

                const openPunctuatorText = leftToken.value

                let nextIndex = index + 1
                while (nextIndex < evaluates.length) {
                    const closeEvaluate = evaluates[nextIndex++]
                    const rightToken = findIndentationEndPunctuator(
                        closeEvaluate,
                        openPunctuatorText
                    )
                    if (!rightToken) {
