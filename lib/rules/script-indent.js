...
module.exports = {
    meta: {
        docs: {
            ...
            url:
                "https://github.com/ota-meshi/eslint-plugin-lodash-template/blob/v0.0.13/docs/rules/script-indent.md",
        },
        fixable: "whitespace",
        schema: [
            {
    ...
    create(context) {
        ...
        function getTopLevelIndentByTemplateTag(templateTag) {
            const text = getLineText(templateTag.loc.start.line)
        ...
        function* genCollectNodeList(nodeList, leftToken, rightToken) {
            ...
            for (const node of nodeList) {
                ...
                // Collect related tokens.
                // Commas between this and the previous, and the first token of this node.
                for (const t of genCollectTokens(elementTokens.firstToken)) {
                    yield t
                }
            ...
            if (rightToken) {
                for (const t of genCollectTokens(rightToken)) {
                    yield t
                }
            }
        }
            ...
            function validateBaseIndent(line, actualText, expectedIndent) {
                ...
                if (
                    ...
                    context.report({
                        ...
                        message:
                            "Expected base point indentation of {{expected}}, but {{actualPhrase}}.",
                        data: {
            ...
            offsets.forEach((value, line) => {
                ...
                if (actualRelativeIndent !== expectedIndent.indent) {
                    context.report({
                        ...
                        data: {
                            relative: expectedBaseIndent ? " relative" : "",
        ...
        return {
            ...
            "Program:exit"() {
                for (const token of microTemplateService.getMicroTemplateTokens()) {
                    ...
                    const charIndex = token.code.search(/\S/)
