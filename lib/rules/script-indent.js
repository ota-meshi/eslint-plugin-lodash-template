...
module.exports = {
    meta: {
        docs: {
            ...
            url:
                "https://github.com/ota-meshi/eslint-plugin-lodash-template/blob/v0.0.11/docs/rules/script-indent.md",
        },
        fixable: "whitespace",
        schema: [
            {
    ...
    create(context) {
        ...
        function getTopLevelIndentByTemplateTag(templateTag) {
            const beforeText = getBeforeTextInLine(templateTag.loc.start)
            if (beforeText.trim()) {
                return NaN
            }
            return beforeText.length + options.indentSize * options.startIndent
        }
        ...
        function processNodeList(
            ...
            if (nodeList.length >= 1) {
                ...
                for (const node of nodeList) {
                    ...
                    // Collect related tokens.
                    // Commas between this and the previous, and the first token of this node.
                    for (const t of genCollectTokens(
