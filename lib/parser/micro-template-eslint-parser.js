...
/**
 * Check whether the code is a HTML or target file.
 * @param {string} code The source code to check.
 * @param {object} options The parser options.
 * @returns {boolean} `true` if the source code is a HTML or target file.
 */
function isParseTargetFile(code, options) {
    const filePath =
        typeof options.filePath === "string" ? options.filePath : "unknown.js"

    if (container.isIgnoreFile(filePath)) {
        return false
    }

    if (container.isParseTargetFile(filePath)) {
        return true
    }
...
function parseForESLint(code, options) {
    const parserOptions = normalizeOptions(options)
    if (!isParseTargetFile(code, parserOptions)) {
