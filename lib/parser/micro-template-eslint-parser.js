...
function settingToRegExpInfo(val, defaultDelimiters) {
    if (!val) {
        return {
            source: `${defaultDelimiters[0]}([\\s\\S]*?)${
                defaultDelimiters[1]
            }`,
...
function* genMicroTemplateTokens(code, options, tokenBuilder) {
    ...
    const re = RegExp(
        [escapeInfo.source, interpolateInfo.source, evaluateInfo.source].join(
            "|"
        ),
        "g"
    )
...
function parseTemplate(code, parserOptions) {
    ...
    for (const token of genMicroTemplateTokens(
        ...
        template += `${scriptBeforeBase.replace(
            /\S/g,
            "_"
        )}${token.code.replace(/\S/g, "+")}${scriptAfterBase.replace(
            /\S/g,
            "_"
        )}`
