...
function buildChildTokens(node, parentToken, tokenBuilder, lastIndex) {
    return node.childNodes.map(child =>
        parse5NodeToToken(child, tokenBuilder, lastIndex, {
            parent: parentToken,
        })
    )
}

// invalid Element locations from parse5 ?
//eslint-disable-next-line require-jsdoc
function getElementStartOffset(node) {
    const location = node.__location
    const startOffset = location.startOffset
    if (startOffset || startOffset === 0) {
        return startOffset
    }
    return location.startTag.startOffset
}

//eslint-disable-next-line require-jsdoc
function getEndTagStartOffset(node, token) {
    const location = node.__location
    if (location.endTag) {
        const startOffset = location.endTag.startOffset
        if (startOffset || startOffset === 0) {
            return startOffset
        }
    }
    let startOffset = location.startTag.endOffset

    for (const child of token.children) {
        startOffset = Math.max(startOffset, child.end)
    }
    return startOffset
}

const TOKEN_BUILDERS = {
    HTMLDocument(node, tokenBuilder, lastIndex) {
        const token = tokenBuilder("HTMLDocument", 0, lastIndex)
    ...
    HTMLDocumentFragment(node, tokenBuilder, lastIndex) {
        const token = tokenBuilder("HTMLDocumentFragment", 0, lastIndex)
        token.children = buildChildTokens(node, token, tokenBuilder, lastIndex)
        return token
    },
    ...
    HTMLElement(node, tokenBuilder, lastIndex) {
        ...
        const token = tokenBuilder(
            "HTMLElement",
            location ? getElementStartOffset(node) : 0,
        ...
        if (location.endTag) {
            token.endTag = tokenBuilder(
                "HTMLEndTag",
                getEndTagStartOffset(node, token),
                location.endOffset,
                { parent: token }
            )
        }
