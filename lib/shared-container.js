...
const ignoreTargetContainer = new Map()
...
module.exports = {
    ...
    addIgnoreFile(filename, checker) {
        const arr = computeIfAbsent(ignoreTargetContainer, filename, () => [])
        arr.push(checker)
    },
    isIgnoreFile(filename) {
        for (const checkers of ignoreTargetContainer.values()) {
    ...
    unregisters(filename) {
        ...
        unregister(ignoreTargetContainer, filename, arr => {
            arr.pop()
